...
The card header renders a `lightning-combobox` (`30 / 60 / 90 / 180 / 365` days) next to the refresh button. The active value is held in a module-level `_currentLookback` field, NOT in the `@api lookbackDays` (which stays as the App Builder-controlled seed). Two getters bridge the gap:

- **`currentLookback`** (string) — used by the combobox's value binding because `lightning-combobox` does string-equality on `option.value`. Returns `_currentLookback` if the user has overridden, else `lookbackDays` clamped into `LOOKBACK_VALUES`.
- **`effectiveLookbackDays`** (int) — the value passed to both Apex calls. Mirrors `currentLookback`'s fallback chain so the dropdown and the server queries always agree.

`connectedCallback` deliberately does NOT seed `_currentLookback` from `@api lookbackDays`. When the LWC sits inside a Lightning tab that activates late, `@api` properties may not be hydrated by the time `connectedCallback` runs; seeding eagerly there can leave the combobox stuck on the fallback value (90) even when App Builder configured something else. Reading `lookbackDays` lazily inside the getter avoids the race.

`handleLookbackChange` clears `webEvents`, `crmEvents`, and `activeSourceFilters` before calling `handleRefresh`, so chips and partial-failure banners from the previous window don't bleed into the new one.

The set of dropdown values lives in module-scope `LOOKBACK_OPTIONS` (with stringified `value` for combobox compatibility) and `LOOKBACK_VALUES` (numeric mirror used for clamping). 365 is the upper bound; it's intentionally aligned with `CrmTimelineController.MAX_LOOKBACK_DAYS` and `DataCloudWebEngagementController.COLD_MAX_LOOKBACK_DAYS` — bump those Apex constants in lockstep if the dropdown ever extends beyond a year.

## Empty-state gating

`isWebLoadedAndEmpty` (renamed from `isFullyLoadedAndEmpty` in 2026-06-01) controls whether the **No recent engagements found.** fallback renders. The current rule is **`!loadingWeb && !hasAnyEvents`** — CRM intentionally NOT in the gate. The previous gate (`!loadingWeb && !loadingCrm && !hasAnyEvents`) had a known failure mode: when web returned empty but CRM was still pending or hung, the fallback was suppressed for the entire CRM round-trip, producing a fully blank card body. The italic *Loading CRM activity...* chip below the fallback already communicates that CRM is in flight, so excluding it from the gate is the right call.
...
- **"Empty timeline = component is broken" — usually no.** First check the runtime lookback dropdown in the card header. If it's set to `30` or `60` days and the Account's most recent activity is older than that, the timeline is correctly empty. The FlexiPage's stored `lookbackDays` value seeds the dropdown — and that value can be invisible in App Builder when the LWC's `.js-meta.xml` schema has been updated since the placement was last opened. To verify, dump the FlexiPage metadata via tooling API (`SELECT Metadata FROM FlexiPage WHERE DeveloperName = '<page>'`) and look at the `componentInstanceProperties` for `webEngagementData`. The "Omega, Inc." trap (2026-06-01): an account with 30 records across the last year showed empty because the FlexiPage XML had `lookbackDays=60` cached and the most recent activity was 64 days old. The runtime dropdown lets users widen the window without an admin round-trip — but if they don't know it exists, the fix is to pop open the FlexiPage and bump the seed.
