...
- SFDX project, `sourceApiVersion` 67.0. Target org `jdo-oe0sdd` (org id `00Dam00000Uo32qEAB`, == `storm-16a17dc388fbe6` — the former `jdo-1lrnov` / `jdo-0pz8au` aliases pointed at this same org but no longer resolve).
...
├── objects/              # CommandCenterConfig__c — the config singleton (per-center JSON + model cache + per-user brand-theme / display-size maps)
...
- **Command-center cockpit view** — the banker home has two layouts behind a top-bar `HomeViewToggle` (`HomeViewProvider`, persisted per persona): the original stacked **Current list** and a **Cockpit** command center. The cockpit brief opens with a personalized time-of-day greeting + AI headline, an embedded **Right Now** card (the top next action, side by side at `lg:grid-cols-[1fr_340px]`), and a slim **Portfolio Pulse** strip; below it, a four-KPI sparkline **vitals** row (Pipeline — now consolidated with open-opps — Leads & Referrals, at-risk, active goals) → **Priority Queue + Recommended Actions** side by side → tabbed customer-360 **`WorkspacePanel`** → full-width **supporting band**. Selection is master-detail: clicking a client/task/opp/meeting anywhere drives the sticky `WorkspacePanel` (`WorkspaceSelection` context; a localStorage `WorkspaceSelectionProvider` holds pinned accounts). The cockpit is **de-duplicated, sidebar-first**: the bottom detail boxes were removed, so the KPI vitals, the Portfolio-Pulse header, and the left `CommandRail`'s section links all open a `DataExplorerModal` (search/filter/table) — explorers cover activity / pipeline-movement / at-risk / agenda / opportunities / **leads** / **goals** / **lifeEvents**; a row there opens a stacked read-only `DetailModal`. Because the cockpit drops the on-page anchors those rail links used to scroll to, `WorkspaceSelection` carries a `SectionNavRequest` bridge: the rail scrolls when an anchor exists (classic view) and otherwise raises a nav intent the page maps via `NAV_TO_EXPLORER` to the matching modal — which keeps `HomePage.tsx` byte-identical across the three persona bundles. The Priority Queue renders through `PriorityQueueCard`, fed by the shared **`buildPriorityQueue`** blender (`_shared/src/data/priorityQueue.ts`) — it merges the persona's signature risk feed with opportunity- and overdue-task-derived items, each carrying a signal-native `dueDate` so severities interleave when sorted by due date rather than clustering by severity.
...
- **Per-user brand theming & display size** — a theming UI builds, names, edits, and activates holistic **brand themes** (multi-color logo palette via `paletteExtract` + complementary suggestion, per-element/role color mapping with hex/RGB `ColorInput`, an optional dedicated **AI accent** as a third brand color, agent-bubble + aurora-wash colors, plus fixed always-available **Light/Dark** baselines) and picks an app-wide **display size** (font/UI scale). The token machinery lives in `_shared/src/theme/` (`brandThemes`, `activeBrand`, `applyActiveTheme`, `defaultThemes`, `displaySize`, `paletteExtract`); `applyActiveThemeOnLoad()` runs once in each `app.tsx` (fire-and-forget — renders the persona default first, swaps to the brand on resolve, never blocks paint). Storage split: theme **definitions** are org-shared (`Theme_Library__c`), while the **active theme** and **display size** are per user — UserId-keyed maps in `Active_Themes__c` / `Display_Sizes__c` on the same `CommandCenterConfig__c` singleton. Same apexrest-bridge constraint as the rest of the app: `brandThemeClient` reads/writes through `CommandCenterConfigRest` routes `/config/themes`, `/config/active-theme`, `/config/display-size`, and `/config/brand-logo?url=` (server-side logo fetch → base64, since gstatic faviconV2 replaced the now-301'ing google s2 favicon endpoint). FLS for the three new fields is granted by the `CommandCenterConfigAdmin` permission set.
