...
| `CumulusMSCIESG` | 34,170 | **Commercial** — corporate-entity ESG (not per-holding) | ✅ shipped (Company Intel tab) |
...
| `CumulusZoomInfoFirmographics` | 34,170 | **Commercial** — firmographics | ✅ shipped (Company Intel tab) |
| `CumulusBoardExExecIntel` | 957,141 | **Commercial** — exec intel | ✅ shipped (Company Intel tab) |
| `CumulusSynthRelationshipGraph` | 61,769 | All — relationship/household graph | Partial (network tab) → **enrich** |
| `SEC_Filings` | 11 | Commercial — filings | ✅ shipped (Company Intel tab) |
...
| `Loan_Delinquencies` | 750 | Delinquency risk | ✅ shipped (book-level Home panel — not customer-joinable) |
