# React Wealth — Wealth Management Advisory Desk

The **Wealth Management** persona of the React-Headless cockpit suite: a wealth advisor's advisory desk + customer 360, built as a Salesforce Multi-Framework **UI Bundle** (React 19 + Vite 7 + TypeScript + Tailwind v4 / shadcn/ui). Styled with the **Cumulus Aurora** design language on a **gold** accent (`#d4af37`), tagline *Advisory Desk*.
...
From this bundle directory (`force-app/main/default/uiBundles/ReactWealth`):
...
sf project deploy start \
  ...
  --source-dir force-app/main/default/applications/ReactWealth.app-meta.xml \
  --source-dir force-app/main/default/permissionsets/ReactWealth_Access.permissionset-meta.xml \
  -o jdo-oe0sdd --json
```

Always capture `--json` and read `status` / `numberComponentErrors`. The app renders at the **Salesforce App Domain** (`https://storm-16a17dc388fbe6--c.demo.my.salesforce.app/app/c__ReactWealth`), **not** `/lightning/app/<name>`. See the project [DEPLOYMENT_GUIDE.md](../../../../../docs/DEPLOYMENT_GUIDE.md) for the App Launcher tile bridge and the beta delete-and-redeploy migration.
