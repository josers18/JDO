...
actual_accts AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM DATA_JEDAIS.FINS__PUBLIC.PLAID_HELD_AWAY
)
SELECT (SELECT n FROM expected)     AS expected_accts,
       (SELECT n FROM actual_accts) AS actual_accts,
       (SELECT COUNT(*) FROM DATA_JEDAIS.FINS__PUBLIC.PLAID_HELD_AWAY) AS total_rows,
       (SELECT COUNT(*) FROM DATA_JEDAIS.FINS__PUBLIC.PLAID_HELD_AWAY) BETWEEN
