...
- The renderer is **markdown only** — if your prompt template emits HTML (`<p>...</p>`) instead, the renderer will escape it as text. Either change the prompt to emit markdown, or use sibling project `DC_AgentForce_Output_LWC/` (Apex sanitizer + `marked.js`).
