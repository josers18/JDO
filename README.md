...
[![Agentforce Renderer](https://img.shields.io/badge/Agentforce_Renderer-Markdown_%2B_HTML-7F56D9?style=for-the-badge)](DC_AgentForce_Markdown_Renderer/docs/INTEGRATION_GUIDE.md)
...
| [**DC_AgentForce_Markdown_Renderer**](DC_AgentForce_Markdown_Renderer/README.md) | **Markdown / HTML renderer** for Agentforce GenAI Function output. Wired in via Lightning Type `c__markdownResponse` + renderer override (no Flow plumbing or Apex sanitizer). Dual-input: auto-detects markdown vs. HTML and routes through regex parser or DOMParser-based sanitizer (tag/attribute allowlist + URL allowlist on links + `rel=noopener noreferrer` + control-char strip). 56 Jest tests. | [README](DC_AgentForce_Markdown_Renderer/README.md) · [INTEGRATION_GUIDE](DC_AgentForce_Markdown_Renderer/docs/INTEGRATION_GUIDE.md) · [AGENTS](DC_AgentForce_Markdown_Renderer/AGENTS.md) |
| [**Cumulus_Assistant**](Cumulus_Assistant/README.md) | **Agentforce agent bundle** for the Cumulus Bank assistant — 3.3K-line `.agent` definition (18+ topic subagents covering Cases, communication, post-meeting follow-up, GeneralCRM, RecordManagement, ProductOffers, FinancialAccounts, ClientInsights, Referrals, etc.) + DC_Product_Offers GenAiFunction (typed `c__markdownResponse`, consumes the renderer above). | [README](Cumulus_Assistant/README.md) · [AGENTS](Cumulus_Assistant/AGENTS.md) |
