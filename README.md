...
| [**DC_AgentForce_Markdown_Renderer**](DC_AgentForce_Markdown_Renderer/README.md) | **Markdown renderer** for Agentforce GenAI Function output. Wired in via Lightning Type `c__markdownResponse` + renderer override (no Flow plumbing or Apex sanitizer). Regex parser handles headings, bold/italic, lists, blockquotes, code, tables, links. Security: URL scheme allowlist on links + `rel=noopener noreferrer` + control-char strip on URL paths. 38 Jest tests. | [README](DC_AgentForce_Markdown_Renderer/README.md) · [INTEGRATION_GUIDE](DC_AgentForce_Markdown_Renderer/docs/INTEGRATION_GUIDE.md) · [AGENTS](DC_AgentForce_Markdown_Renderer/AGENTS.md) |
