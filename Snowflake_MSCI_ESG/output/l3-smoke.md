...
```sql
WITH expected AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM DATA_JEDAIS.FINS__PUBLIC.V_ACCOUNT_ANCHORS WHERE ACCOUNT_TYPE_FLAG = 'BUSINESS'
),
actual AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM DATA_JEDAIS.FINS__PUBLIC.MSCI_ESG_SCORES
)
...
```sql
SELECT MSCI_ESG_RATING, COUNT(*) as cnt FROM DATA_JEDAIS.FINS__PUBLIC.MSCI_ESG_SCORES GROUP BY 1 ORDER BY 2 DESC
```
...
```sql
SELECT INDUSTRY_CLASSIFICATION, COUNT(*) as cnt FROM DATA_JEDAIS.FINS__PUBLIC.MSCI_ESG_SCORES GROUP BY 1 ORDER BY 2 DESC
```
...
```sql
SELECT CONTROVERSY_FLAG_COUNT, COUNT(*) as cnt FROM DATA_JEDAIS.FINS__PUBLIC.MSCI_ESG_SCORES GROUP BY 1 ORDER BY 1
```
...
```sql
SELECT LAST_RATING_CHANGE_DIRECTION, COUNT(*) as cnt, pct FROM DATA_JEDAIS.FINS__PUBLIC.MSCI_ESG_SCORES GROUP BY 1
```
