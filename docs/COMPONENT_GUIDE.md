...
| Real Time Digital Engagements | `c/webEngagementData` | Record (Account, Contact) | **Promise A:** `DataCloudWebEngagementController.getWebEngagementData` (Data Cloud Data Graph via `callout:Data_Cloud_API`, default graph `RT_Web_Engagementsv2`, configurable per-instance via **Data Graph API name**) **+ Promise B:** `CrmTimelineController.getCrmTimelineEvents` (parallel SOQL across **Cases / Tasks (incl. logged calls) / Calendar Events / Agentforce VoiceCalls**, opt-in per source via App Builder toggles, shared `lookbackDays` window, per-source `LIMIT 200`). Style B card stream with chip filters, day groups, source-colored left rails, expand-on-click details, partial-failure retry banners. 10 App Builder properties total. | [Web_Engagements_RT_Timeline/README.md](../Web_Engagements_RT_Timeline/README.md) |
