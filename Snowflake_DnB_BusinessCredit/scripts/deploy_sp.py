#!/usr/bin/env python3
"""Deploy SP_GENERATE_DNB_BUSINESS_CREDIT into DATA_JEDAIS.FINS__PUBLIC.

Generator-style sibling of Plan 2's `Snowflake_MSCI_ESG/scripts/deploy_sp.py`.
Plan 1 hand-built `procedures/sp_create_procedure.sql` once; Plan 2 automated
that step; Plan 3 reuses the recipe with a per-dataset identifier swap so
future datasets can clone with zero manual SQL editing.

What this script does:
  1. Read the SP body from `procedures/sp_generate_dnb_business_credit.py`.
  ...
  5. Run `snow sql -c <connection> -f procedures/sp_create_procedure.sql`.

Why "DnB" (not "D&B") in the SQL header comments:
  Plan 3 T2 surfaced a `snow sql` Jinja templating gotcha — `&` in SQL files
  trips the templating engine. The README/AGENTS use the canonical "D&B" /
  "Dun & Bradstreet" branding; this script emits "DnB" / "Dun and Bradstreet"
  in the generated SQL header so `snow sql -f` runs cleanly.
...
Usage:
    python scripts/deploy_sp.py [--connection GSB13421] [--no-deploy]
"""
...
SP_PY = REPO_ROOT / "procedures" / "sp_generate_dnb_business_credit.py"
...
PROCEDURE_FQN = "DATA_JEDAIS.FINS__PUBLIC.SP_GENERATE_DNB_BUSINESS_CREDIT"
TABLE_FQN = "DATA_JEDAIS.FINS__PUBLIC.DNB_BUSINESS_CREDIT"
...
-- Task:    Plan 3 T6
-- Source:  procedures/sp_generate_dnb_business_credit.py
...
-- Salt:     "{DATASET_SALT}" (per-row), "duns_id" (year-stable DUNS)
-- Table:    {TABLE_FQN}
...
def build_sql() -> str:
    ...
    # `snow sql -f` runs the file through Jinja, and `&<identifier>` parses as
    # a (deprecated) variable reference. The SP source uses canonical "D&B" in
    # docstrings; rewrite to "DnB" only in the generated SQL, leaving the
    # Python source untouched. Spaced ampersands (e.g. "Food & Beverage",
    # "Oil & Gas") are NOT rewritten — they're not followed by an identifier
    # and Plan 2 proved they deploy fine.
    sp_clean = sp_clean.replace("D&B", "DnB")
...
def main() -> int:
    ...
    verify_cmd = (
        ...
        + " -q \"SHOW PROCEDURES LIKE 'SP_GENERATE_DNB_BUSINESS_CREDIT' IN SCHEMA DATA_JEDAIS.FINS__PUBLIC\""
    )
