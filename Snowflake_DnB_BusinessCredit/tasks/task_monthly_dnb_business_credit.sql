-- =============================================================================
-- DATA_JEDAIS.FINS__PUBLIC.TASK_MONTHLY_DNB_BUSINESS_CREDIT
...
-- Wrapper:  DATA_JEDAIS.FINS__PUBLIC.SP_RETRY_WRAPPER('DATA_JEDAIS.FINS__PUBLIC.SP_GENERATE_DNB_BUSINESS_CREDIT()', 2)
--
-- Pattern parity:
--   Mirrors TASK_MONTHLY_MSCI_ESG_SCORES + TASK_MONTHLY_CLARITAS_DEMOGRAPHICS
--   (DATA_JEDAIS.FINS__PUBLIC) — same warehouse (MAIN_WH_XS), same cron, same SP_RETRY_WRAPPER
...
CREATE OR REPLACE TASK DATA_JEDAIS.FINS__PUBLIC.TASK_MONTHLY_DNB_BUSINESS_CREDIT
    WAREHOUSE = MAIN_WH_XS
    SCHEDULE  = 'USING CRON 0 7 1 * * UTC'
AS
    CALL DATA_JEDAIS.FINS__PUBLIC.SP_RETRY_WRAPPER(
        'DATA_JEDAIS.FINS__PUBLIC.SP_GENERATE_DNB_BUSINESS_CREDIT()',
        2
    );

ALTER TASK DATA_JEDAIS.FINS__PUBLIC.TASK_MONTHLY_DNB_BUSINESS_CREDIT RESUME;
