...
WITH expected AS (
    ...
    FROM FINS.PUBLIC.V_ACCOUNT_ANCHORS WHERE ACCOUNT_TYPE_FLAG = 'BUSINESS'
),
actual AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT
)
...
```sql
SELECT COUNT(*) AS NULL_DUNS_CNT FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT
  WHERE DUNS_NUMBER IS NULL OR LENGTH(DUNS_NUMBER) <> 9
-- 0

SELECT COUNT(*) AS NULL_PARENT_FOR_NONSTAND FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT
  WHERE CORPORATE_FAMILY_SIZE > 1 AND ULTIMATE_PARENT_DUNS IS NULL
-- 0

SELECT COUNT(*) AS NONNULL_PARENT_FOR_STANDALONE FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT
...
```sql
SELECT FINANCIAL_STRENGTH_TIER, COUNT(*) AS CNT FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT GROUP BY 1 ORDER BY CNT DESC
```
...
```sql
SELECT COMPOSITE_RISK_SCORE, COUNT(*) AS CNT FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT GROUP BY 1 ORDER BY 1
```
...
```sql
SELECT SUPPLIER_RISK_LEVEL, COUNT(*) AS CNT FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT GROUP BY 1 ORDER BY CNT DESC
```
...
```sql
SELECT VERIFICATION_STATUS, COUNT(*) AS CNT FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT GROUP BY 1 ORDER BY CNT DESC
```
...
SELECT BUCKET, COUNT(*) FROM (
  SELECT CASE WHEN CORPORATE_FAMILY_SIZE = 1 THEN '1 (standalone)'
              ...
              ELSE '100+' END AS BUCKET FROM FINS.PUBLIC.DNB_BUSINESS_CREDIT
) GROUP BY 1
```
