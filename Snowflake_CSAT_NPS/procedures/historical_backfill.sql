...
accounts AS (
    SELECT DISTINCT ACCOUNT_ID FROM DATA_JEDAIS.FINS__PUBLIC.MASTER_ACCOUNTS
),
...
filtered AS (
    ...
    FROM with_nps w
    WHERE NOT EXISTS (
        SELECT 1 FROM DATA_JEDAIS.FINS__PUBLIC.CSAT_NPS_DATA e
        WHERE e.ACCOUNTID = w.ACCOUNT_ID AND e.SCORE_DATE = w.score_date
    )
)
SELECT
    (SELECT COALESCE(MAX(ROWID), 0) FROM DATA_JEDAIS.FINS__PUBLIC.CSAT_NPS_DATA) + ROW_NUMBER() OVER (ORDER BY ACCOUNT_ID, score_date),
    ACCOUNT_ID,
    NULL,
    csat_score,
    csat_description,
