...
### SP Call Result
```
CALL FINS.PUBLIC.SP_GENERATE_CLARITAS_DEMOGRAPHICS()
...
```sql
WITH expected AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM FINS.PUBLIC.V_ACCOUNT_ANCHORS WHERE ACCOUNT_TYPE_FLAG = 'PERSON'
),
actual AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM FINS.PUBLIC.CLARITAS_DEMOGRAPHICS
)
SELECT
