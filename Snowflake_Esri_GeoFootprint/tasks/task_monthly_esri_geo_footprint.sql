-- =============================================================================
-- FINS.PUBLIC.TASK_MONTHLY_ESRI_GEO_FOOTPRINT
...
-- Wrapper:  FINS.PUBLIC.SP_RETRY_WRAPPER('FINS.PUBLIC.SP_GENERATE_ESRI_GEO_FOOTPRINT()', 2)
--
-- Pattern parity:
--   Mirrors TASK_MONTHLY_MSCI_ESG_SCORES + TASK_MONTHLY_CLARITAS_DEMOGRAPHICS
--   + TASK_MONTHLY_DNB_BUSINESS_CREDIT (FINS.PUBLIC) — same warehouse
...
CREATE OR REPLACE TASK FINS.PUBLIC.TASK_MONTHLY_ESRI_GEO_FOOTPRINT
    WAREHOUSE = MAIN_WH_XS
    SCHEDULE  = 'USING CRON 0 7 1 * * UTC'
AS
    CALL FINS.PUBLIC.SP_RETRY_WRAPPER(
        'FINS.PUBLIC.SP_GENERATE_ESRI_GEO_FOOTPRINT()',
        2
    );

ALTER TASK FINS.PUBLIC.TASK_MONTHLY_ESRI_GEO_FOOTPRINT RESUME;
