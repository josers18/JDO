...
| C9 | ✅ **Closed by acceptance 2026-05-12 — confirmed structural FSC managed-package limitation.** Deep probe of the FSC FinServ namespace eliminated every programmatic path: (1) **Direct DML reverts silently** — `FinancialAccountTrigger` (FSC-packaged) overwrites `FinServ__Household__c` back to null in `before update`; `Database.update` reports `isSuccess=true` with 0 errors but a same-transaction re-query shows null. (2) **No `Account.FinServ__Household__c` field exists** on the member side in this org — household membership is ACR-driven, not field-driven; the "set on PA, cascade to FAs" path the audit hypothesized is structurally impossible. (3) **`AccountContactRelation` touch** (re-saving the ACR row) does not trigger FA household derivation. (4) **`FinServ.GroupAssignmentBatchable`** (the FSC-blessed batch class — its constructor refused our IDs and the runtime error message pointed us to it) executes cleanly but doesn't backfill — its scope is rollup recalculation on already-linked FAs, not derivation. (5) **`FinServ.HouseholdAssignmentBatchable.runJob('FinServ__FinancialAccount__c')`** (the alternative class) refused with *"Record Rollup Optimization (Beta) is disabled. Use GroupAssignmentBatchable batch job"*. (6) **Tested `FinServ__Record_Rollup_Configuration__c.FinServ__Skip_Record_Rollup_Triggers__c=false`** for the Financial Account row: still no effect on direct DML or batch behavior; restored to `true` after probe. **`FscHouseholdLookupBackfill` Apex/Test/permset remain deployed** — usable when an unblocking path is found. **Demo impact accepted:** 90% of household RBL rollups read $0; demos that emphasize household-360 net-worth views need to use Smith/Morris/Bennett/Blake households (the 4 that have working rollups across the 48 linked FAs) until a fix is found. **Reopen criteria:** (a) Salesforce releases a public `FinServ.HouseholdServices` API for household assignment; (b) Salesforce Support provides the FSC-internal trigger-bypass mechanism; (c) demo arc requires household rollups on a household not in the 4 working ones (workaround: use the UI "Update Household" action one PA at a time — the trigger's blessed path). |
...
| H11 | ✅ **Done 2026-05-12.** Bulk-canceled all 36 InProgress orchestrations via `Status='Canceled'` (worked cleanly with no FSC-trigger interference); the 35 remaining paused FlowInterviews **auto-deleted as a cascade of orchestration cancellation** (no separate interview-delete step needed — the 36th interview was the one our 1-record probe canceled first). Also cleaned up 3 unrelated Error interviews from today (Data Cloud profile-update DUPLICATE_VALUE × 2 + B5-induced Apex CPU exceeded × 1). Then re-attempted A4-deferred Obsolete flow deletes: **5 deleted** (Onboarding: Process a Party Profile, Orchestrate Fee Reversal, Orchestrate Transaction Dispute, Process Address Update, Set VoiceCall Subject). **6 still blocked** but now triaged as separate findings: 5 are managed-package undeletables (Agent Configuration Review Generator, Create Agent, Create Agent Test Cases, Generate Spec, VRA Manage Lobby Invites — these were always going to stay), and 5 (DC_Attrition_Explanation ×2, DC_CSAT_Prompt, DC_Mask, DC_Summarize_Financial_Transactions) are referenced by Generative AI Prompt Template Versions — a *separate* dependency the audit hadn't surfaced. 1 (Case_CSAT_NPS_Survey) reports Obsolete-listed but Active-blocked-on-delete, needs deactivation first. The "11 blocked by orchestrations" assumption was wrong: only 4 actually were. Net flow-cleanup count: **256/262 = 98%**. Remaining 6 are noted as Phase A4 part 3 backlog (Prompt Template references + 1 active deactivation) but low-value vs. effort. |
...
| A7 | ✅ **Done 2026-05-12 (1 uninstall + 6 deferred to UI + 4 reclassified).** Inventory done 2026-05-11; uninstall pass executed 2026-05-12 against the 11 SUNSET + OLD_DEMO candidates. **1 actually uninstalled: B2B LE Mood Board** (no live references). **6 require Setup-UI uninstall** because the CLI doesn't support 1GP-package uninstalls — the 5 SUNSET packages (Quip, Quip Connected App, SalesforceIQ Inbox, SalesforceIQ Cloud, Sales Insights/`OIQ`) and `b2bmaIntegration`. CLI returns *"You're trying to uninstall a first-generation managed package, ID: …. You can uninstall this package type only in the Salesforce user interface."* — these still belong on the uninstall-via-UI list; ~2 min each in Setup → Installed Packages. **4 reclassified from OLD_DEMO to "in active use" (do NOT remove):** B2B LE Bundle Item is blocked by 3 active flows (B2B_Bundle_Items_Create_Unique_Junction_Flow_2_0, Subflow_Activate_Order_with_Bundle_Items_2_0, B2B_LE_Checkout_with_Bundle); B2B LE Cart Upload, B2B LE Cross-Sell, and B2B LE Video Player are referenced by current SDO Commerce demo sites (`SDO - B2B Commerce Enhanced` and `SDO - Commerce for B2B2C`). These 4 components were *repurposed* by the newer SDO Commerce profiles rather than retired — the audit's OLD_DEMO classification was wrong for them. Inventory doc should mark them KEEP. **Remaining work for a UI session:** the 6 1GP uninstalls (~12 min total). The 4 in-use packages stay. MKTG_LEGACY (3) unchanged — still pending per-package review. | Medium | L |
