...
| A7 | ✅ **Closed 2026-05-12.** Inventory done 2026-05-11; uninstall pass executed 2026-05-12 against the 11 SUNSET + OLD_DEMO candidates. **Net result: 1 uninstalled (B2B LE Mood Board), 4 reclassified to KEEP (in active use), 6 deferred indefinitely.** The 4 reclassified packages — B2B LE Bundle Item (blocked by 3 active flows), B2B LE Cart Upload, B2B LE Cross-Sell, B2B LE Video Player (referenced by current SDO Commerce demo sites: `SDO - B2B Commerce Enhanced`, `SDO - Commerce for B2B2C`) — were repurposed by newer SDO Commerce profiles rather than retired with the B2B LE arc; the inventory's namespace-only check missed those references. The 6 deferred (Quip, Quip Connected App, SalesforceIQ Inbox, SalesforceIQ Cloud, Sales Insights/`OIQ`, `b2bmaIntegration`) are 1GP packages requiring Setup-UI uninstall (CLI rejects with *"You can uninstall this package type only in the Salesforce user interface"*); estimated ~12 min in Setup → Installed Packages. **Decision 2026-05-12:** the 6 UI uninstalls and the remaining MKTG_LEGACY (3) / UTILITY (21) / UNCLASSIFIED (29) reviews are not worth the per-package effort for the marginal hygiene gain — packages stay as-is. **Reopen criteria:** an FSC upgrade or major demo blocked by one of these packages, or a future hygiene initiative that explicitly targets installed-package count as a metric. The audit's namespace-only inventory check is captured in `cumulus-package-inventory.md` for that future pass. | Medium | L |
