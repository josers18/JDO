...
```sql
WITH expected AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM DATA_JEDAIS.FINS__PUBLIC.V_ACCOUNT_ANCHORS
),
actual AS (
    SELECT COUNT(DISTINCT ACCOUNT_ID) AS n FROM DATA_JEDAIS.FINS__PUBLIC.WORLD_CHECK_AML
)
