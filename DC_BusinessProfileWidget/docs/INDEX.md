# Documentation index — DC Business Profile Widget

Overview: **[README.md](../README.md)**

## Read first

| Order | Document | Audience | Contents |
|-------|----------|----------|----------|
| 1 | [DEPLOY.md](DEPLOY.md) | Admin / developer | Deploy command, what ships |
| 2 | [SETUP.md](SETUP.md) | Admin | Apex access, remote sites, Lightning page |
| 3 | [HOW_TO.md](HOW_TO.md) | Admin / analyst | Field maps, Flows, themes |

## Deeper topics

| Document | Audience | Contents |
|----------|----------|----------|
| [FLOW_GUIDE.md](FLOW_GUIDE.md) | Flow builders | `flow:` tokens, assembly vs Insight |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Technical readers | Sequence and merge order |
| [COMPONENT_REFERENCE.md](COMPONENT_REFERENCE.md) | App Builder admins | All `@api` properties |
| [APEX_REFERENCE.md](APEX_REFERENCE.md) | Developers | `getProfileData`, `generateSummary` |
| [PROMPT_TEMPLATE.md](PROMPT_TEMPLATE.md) | Einstein admins | JSON sent to prompt template |
| [DIAGRAMS.md](DIAGRAMS.md) | Reviews | Mermaid diagrams |

## When something fails

| Document | Use when |
|----------|----------|
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Blank fields, map errors, deploy issues |

## Inventory

| Resource | Use when |
|----------|----------|
| [artifacts.md](../artifacts.md) | Checklist of metadata |
| [GIT.md](GIT.md) | Monorepo path |

## Theme reference (PDF)

| Resource | Use it when |
|----------|-------------|
| [Widget theme catalog (PDF)](../../docs/assets/widget_theme_catalog.pdf) | **Visual catalog** of **42 themes** (same names as **Theme mode** in App Builder). Also linked from the monorepo hub: [THEME_CATALOG.md](../../docs/THEME_CATALOG.md). |
