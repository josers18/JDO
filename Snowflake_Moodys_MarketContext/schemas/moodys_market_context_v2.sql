...
--             (T7 will re-create the data stream / DLO / DMO against this DDL).
...
CREATE OR REPLACE TABLE FINS.PUBLIC.MOODYS_MARKET_CONTEXT (
    ...
    CONSTRAINT pk_moodys_market_context PRIMARY KEY (ACCOUNT_ID, PROFILE_DATE)
)
COMMENT = 'Moody''s Investors Service / Moody''s Analytics-style synthetic credit-rating + market-context dataset per Cumulus BUSINESS account, daily snapshot. v2 re-scope from instrument-scoped (Plan 13 v1) to per-BUSINESS-account (Plan 13 v2). PK is (ACCOUNT_ID, PROFILE_DATE); joinable to ssot__Account__dlm via ACCOUNT_ID. ~11,389 BUSINESS anchors * 90 daily snapshots = ~1,025,010 rows after backfill; daily SP MERGE-replaces in place at ~11,389 rows/day. Two-salt model: "moodys" (daily) for market signals + "moodys_year" (year-stable) for editorial signals (rating, outlook, 52-week band, liquidity tier, revenue multiple). 14 NOT NULL + 2 NULLable (OUTLOOK_LAST_CHANGED_DATE, ANNUAL_REVENUE_USD pass-through; plus EMPLOYEE_COUNT pass-through NULLable). 0 BOOLEAN — no DC Boolean-declaration ceremony required. INDUSTRY-driven rating bias + ANNUAL_REVENUE-driven market-cap synthesis. See Snowflake_Moodys_MarketContext/README.md and Plan 13.';
