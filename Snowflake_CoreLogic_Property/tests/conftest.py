...
Snowflake_Cumulus_Common and provides Plan 5 PERSON+ZIP audience overrides."""
...
def in_audience_anchors(all_anchors):
    """Plan 5 audience: ACCOUNT_TYPE_FLAG = 'PERSON' AND POSTAL_CODE non-empty."""
    return [a for a in all_anchors
            if a["ACCOUNT_TYPE_FLAG"] == "PERSON"
               and a["POSTAL_CODE"] is not None
               and a["POSTAL_CODE"] != ""]


@pytest.fixture
def out_of_audience_anchors(all_anchors):
    """Anchors that fail the predicate (BUSINESS, OR PERSON without ZIP)."""
    return [a for a in all_anchors
            if a["ACCOUNT_TYPE_FLAG"] != "PERSON"
               or a["POSTAL_CODE"] is None
               or a["POSTAL_CODE"] == ""]
