 ...
 *
 * 43-theme dict keyed on themeMode strings. Inlined here (not imported from a
 * sibling LWC) so this LWC deploys standalone without depending on the rest
 * of the JDO record-page suite.
 *
 * Source palette parity: keep the 43 base palettes IDENTICAL to
 *   DC_Multiclass_Prediction_LWC/.../predictionThemes.js
 *   DC_Prediction_Model_LWC/.../predictionThemes.js
 * If you tune a token in one, tune it in all three. The shared `--wp-*`
 * vocabulary is what lets `themeMode` stay coherent across record-page LWCs.
 *
 * Three cockpit-specific tokens are LAYERED on every theme by deriveCockpitTokens():
 *   --wp-progress-good : terminal color of the green goal-fill gradient
 *   --wp-progress-blue : the opportunity probability bar fill
 *   --wp-rail-pending  : greyed-step circle stroke for planned events
 */
...
const BASE_THEMES = {
    ...
    union: {
        ...
        '--wp-org-bg':         'rgba(255,255,255,0.025)',
        '--wp-body-bg':        '#100e06',
        '--wp-hdr-glow1':      'rgba(200,130,40,0.20)',
...
/**
 * Per-theme cockpit additions:
 *   --wp-progress-good : the gold→green gradient terminal color (filled goals)
 *   --wp-progress-blue : the opportunity probability bar fill
 *   --wp-rail-pending  : greyed-step circle stroke for planned events
 *
 * Picked by palette mood (light vs dark) rather than per-theme so visual
 * identity stays cohesive across the family.
 */
function deriveCockpitTokens(basePalette) {
    const isDark = isDarkPalette(basePalette);
    return {
        '--wp-progress-good': isDark ? '#3a9a72' : '#2f7d5b',
        '--wp-progress-blue': isDark ? '#5a8fd8' : '#2b5fb3',
        '--wp-rail-pending': basePalette['--wp-border-med'] || (isDark ? 'rgba(255,255,255,0.18)' : '#e1e5ea')
    };
}
...
function isDarkPalette(palette) {
    ...
    // Rec. 709 luminance, threshold tuned for the family palette boundary.
    return 0.2126 * r + 0.7152 * g + 0.0722 * b < 90;
}

export const THEMES = Object.fromEntries(
    Object.entries(BASE_THEMES).map(([key, palette]) => [key, { ...palette, ...deriveCockpitTokens(palette) }])
);
