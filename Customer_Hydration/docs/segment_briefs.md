...
>
> **Phase 6 update (2026-05-27):** Fleet-wide External_ID renumber — 36,222 Accounts moved off `HYDRATE-{RT,WL,SMB,COM,HH}-*` prefixes to a sequential `MDMP##### / MDM#####` convention (persons → `MDMP00001..MDMP25424`, biz → `MDM00001..MDM10798`). The 21 segments below still hold their pre-Phase-6 `External_ID_c__c contains "HYDRATE-"` server-side criteria, so they return 0 members against the renumbered cohort until they're recreated. Code side: `customer_hydration/phase5/segments.HYDRATE_PREFIX` switched to `"MDM"` (matches both MDM##### biz and MDMP##### persons via `contains`). Recreate command (run from a fresh shell — the current Claude session has a token-redaction hook that breaks DC REST auth): `python hydrate.py create-segments --target-org jdo-uqj0jr --recreate '*' --allow-production`. Tracked in [`ROADMAP.md`](ROADMAP.md) § Phase 6 follow-ups.
>
> **Phase 7 update (2026-05-27):** 12 additional field dimensions are now segment-eligible on `ssot__Account__dlm` after the next Account_Home stream refresh — biz cohort: `FinServ__NetWorth__c`, `FinServ__CreditRating__c`, `Tier__c`, `FinServ__LifetimeValue__c`, `FinServ__LastUsedChannel__c`, `Ownership`, `TickerSymbol` (gated to Public, ~9.3% populated); person cohort: `FinServ_Category__pc`, `FinServ_Contact_Status__pc`, `FinServ__CommunicationPreferences__pc`, `FinServ__ContactPreference__pc`, `FinServ__LastUsedChannel__c`. Loaded via `sf data update bulk`; 36,222 / 36,222 successful (post-Email-drop). Future segments can target any of these (e.g. tier-based wealth segments, channel-preference activations, communication-preference suppression). `Email__c` is a formula field (`PersonContact.Email`), by-design NULL on biz and 100% on persons — not a Phase 7 gap. Generator: `scripts/phase7_generate_csvs.py`.
