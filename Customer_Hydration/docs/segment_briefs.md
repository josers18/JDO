...
>
> **Phase 6 update (2026-05-27):** Fleet-wide External_ID renumber — 36,222 Accounts moved off `HYDRATE-{RT,WL,SMB,COM,HH}-*` prefixes to a sequential `MDMP##### / MDM#####` convention (persons → `MDMP00001..MDMP25424`, biz → `MDM00001..MDM10798`). All 21 segments below were DELETE-then-POST recreated against the renumbered cohort the same day (21/21 success, 0 failures) via `python hydrate.py create-segments --target-org jdo-uqj0jr --recreate '*' --allow-production`. Code side: `customer_hydration/phase5/segments.HYDRATE_PREFIX` switched to `"MDM"` (matches both MDM##### biz and MDMP##### persons via `contains`).
>
