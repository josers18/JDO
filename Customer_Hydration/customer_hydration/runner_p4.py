"""Plan 4 runner: Plan 3 + native FSC lineage (Wave F + Wave G).

This is a near-copy of ``runner_p3.py`` extended with two new waves that
load the native FSC standard objects (FinancialAccount, FinancialGoal,
BusinessMilestone, PartyRelationshipGroup, PartyProfile,
ContactPointAddress/Email/Phone, FinancialAccountParty). Plan 5 may
consolidate this with runner_p3 — for now both runners coexist so a
reviewer can ``diff runner_p3.py runner_p4.py`` and see ONLY the
native-lineage additions.

What's new vs runner_p3.py:

  - Phase 3 query-back. After Wave D loads legacy FAs and Goals, the
    runner queries the org for ``{External_ID__c -> Salesforce Id}`` maps
    on ``FinServ__FinancialAccount__c`` and ``FinServ__FinancialGoal__c``.
    These maps drive ``LegacyId__c`` on the native FA / FinancialGoal
    rows (a plain Salesforce Id, NOT an external-id reference).

  - Native CSV generation (8 new CSVs):
      * native_financial_accounts.csv  (HYDRATE-NFA-*, External_ID__c)
      * native_financial_goals.csv     (HYDRATE-NGOAL-*, External_ID__c)
      * business_milestones.csv        (HYDRATE-NMS-*, External_ID__c)
      * party_relationship_groups.csv  (no External_ID__c — insert)
      * party_profiles.csv             (no External_ID__c — insert)
      * contact_point_addresses.csv    (no External_ID__c — insert)
      * contact_point_emails.csv       (no External_ID__c — insert)
      * contact_point_phones.csv       (no External_ID__c — insert)
      * financial_account_parties.csv  (Wave G; no External_ID__c)

  - Wave F: parallel load of the 8 native CSVs above. Pre-load resolver
    rewrites for ``RESOLVE:`` markers on ParentId / AccountId / etc.

  - Post-Wave-F native FA query-back. The native ``FinancialAccount``
    table is queried by ``External_ID__c LIKE 'HYDRATE-NFA-%'`` to build
    a ``HYDRATE-NFA-NNN -> native FA Salesforce Id`` map plus a
    ``HYDRATE-FA-NNN -> HYDRATE-NFA-NNN`` translation map for Wave G.

  - Wave G: load FinancialAccountParty. Pre-load resolver rewrites
    ``RESOLVE-NFA:`` markers (FinancialAccountId) and ``RESOLVE:`` markers
    (AccountId / ContactId).

  - ``--skip-natives`` is FUNCTIONAL. When set:
      * native CSVs are NOT generated (no wasted cycles)
      * Wave F + Wave G are NOT run
      * checkpoint records ``F-skipped`` and ``G-skipped`` so resume
        won't try to re-enter
"""
...
def run_all(args: argparse.Namespace) -> int:
    """Plan 4 runner — Plan 3 waves + native Wave F + G.
    ...
    if args.data_cloud_only:
        print(
            "Plan 4 has no Data Cloud phase yet — use Plan 5 build first.",
            file=sys.stderr,
        )
        return 2
